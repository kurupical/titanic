...
df_agedata = pd.get_dummies(df_train[['Survived','Pclass','Sex','Age','SibSp','Parch','Fare','Embarked']]).dropna().reset_index(drop=True)
X, y = df_agedata.iloc[:, 1:].values, df_agedata.iloc[:, 0].values
lr_agedata, maxscore_subsets_agedata = fit_sbs(X, y, lr_age)

df_noagedata = pd.get_dummies(df_train[['Survived','Pclass','Sex','SibSp','Parch','Fare','Embarked']])
X, y = df_noagedata.iloc[:, 1:].values, df_noagedata.iloc[:, 0].values
lr_noagedata, maxscore_subsets_noagedata = fit_sbs(X, y, lr_noage)

# データ推測
df_key = pd.get_dummies(df_test[['PassengerId','Pclass','Sex','Age','SibSp','Parch','Fare','Embarked']]).dropna(subset=['Age']).reset_index(drop=True)
df_key = df_key['PassengerId']
df_test_agedata = pd.get_dummies(df_test[['Pclass','Sex','Age','SibSp','Parch','Fare','Embarked']]).dropna(subset=['Age']).reset_index(drop=True)
...
df_key = pd.get_dummies(df_test_noagedata[['PassengerId','Pclass','Sex','SibSp','Parch','Fare','Embarked']]).reset_index(drop=True)
df_key = df_key['PassengerId']
df_test_noagedata = pd.get_dummies(df_test_noagedata[['Pclass','Sex','SibSp','Parch','Fare','Embarked']]).reset_index(drop=True)
